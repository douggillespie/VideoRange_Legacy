...
public class AcceptMeasurementDialog extends PamDialog {
	...
	public void setParams() {
		...
		range.setText(String.format("%.1f m", vrMeasurement.locDistance));
		accuracy.setText(String.format("± %.1f m/pix", vrMeasurement.pixelAccuracy));
		
		if (vrMeasurement.imageBearing != null) {
			cameraAngle.setText(String.format("%.1f", vrMeasurement.imageBearing));
...
//			angleValue.setToolTipText("image pos correction only");
		...
		angleCorrection.setText(String.format("%.1f", vrMeasurement.angleCorrection));
...
//		vrMeasurement.comment = comment.getText();
		...
		try {
			vrMeasurement.imageBearing = Double.valueOf(cameraAngle.getText());
		}
		catch (NumberFormatException e) {
