...
import videoRangeLegacy.ImageFileFilter;
...
//			c.anchor = GridBagConstraints.EAST;
			...
			currentPanel=vrControl.getCurrentMethod().getSidePanel();		
...
//			vrControlPanel.validate();
		...
		public void enableArrowButtons(){
			if (vrControl.getVRParams().currShoreFile==null) {
				imageForward.setEnabled(false);
				imageBackward.setEnabled(false);
				return;
			}
			imageForward.setEnabled(vrControl.findNextFile(vrControl.getVRParams().currShoreFile, new ImageFileFilter(), true)!=null);
			imageBackward.setEnabled(vrControl.findNextFile(vrControl.getVRParams().currShoreFile, new ImageFileFilter(), false)!=null);
		}
		...
		private class ArrowListener implements ActionListener{
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (vrControl.getVRParams().currShoreFile==null) return;
				//check the file exists.
				if(!vrControl.getVRParams().currShoreFile.exists()) return; 

				File newFile=vrControl.findNextFile(vrControl.getVRParams().currShoreFile, new ImageFileFilter(), forward);
...
//				vrControl.vrParameters.setCurrentHeightIndex(heights.getSelectedIndex());
		...
		private class SettingsButton implements ActionListener {
			...
			public void actionPerformed(ActionEvent e) {
				vrControl.settingsButton(null,-1);
			}
